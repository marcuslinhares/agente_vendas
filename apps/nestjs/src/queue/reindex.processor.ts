  ...
  async process(job: Job): Promise<void> {
    ...
    for (let i = 0; i < productsToProcess.length; i++) {
      ...
      try {
        ...
        // Generate CLIP embedding via FastAPI
        if (product.imageUrl) {
          try {
            const fastApiUrl = process.env.FASTAPI_URL || 'http://fastapi:8000';
            const { data } = await firstValueFrom(
              this.httpService.post(`${fastApiUrl}/api/embed/product`, {
            ...
            if (data.embedding_clip) {
              embeddingClip = JSON.stringify(data.embedding_clip);
            }
            if (data.error) {
              this.logger.warn(`FastAPI embedding warning for ${product.name}: ${data.error}`);
            }
          } catch (err) {
            this.logger.warn(`Failed to get embeddings for ${product.name}: ${err}`);
          }
        }
