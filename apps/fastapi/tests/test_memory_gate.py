...
async def test_memory_gate_triggers_l3(base_state):
    ...
    mock_response.choices = [
        MagicMock(message=MagicMock(content='{"trigger_l3": true, "reason": "Referenced old info"}'))
    ]
    mock_client.chat.completions.create.return_value = mock_response

    with patch("app.graph.nodes.memory_gate.create_llm_client", return_value=mock_client):
        with patch("app.graph.nodes.memory_gate.settings.openai_api_key", "test-key"):
...
async def test_memory_gate_skips_when_no_api_key(base_state):
    ...
    with patch("app.graph.nodes.memory_gate.settings.openai_api_key", None):
        with patch("app.graph.nodes.memory_gate.settings.openrouter_api_key", None):
            result = await node.run(base_state)
